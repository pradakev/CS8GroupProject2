...
node<ITEM_TYPE>* _PreviousNode(node<ITEM_TYPE>* head,node<ITEM_TYPE>* prevToThis){
    ...
    while(walker!=nullptr&& walker->next != prevToThis){
        walker = walker->next;
    }
    assert(walker!=NULL);
    return walker;
...
template <typename ITEM_TYPE>
node<ITEM_TYPE>* _InsertSorted(node<ITEM_TYPE>* &head,ITEM_TYPE target, bool){
    // this function finds where the node should go, then if it is a empty list
    // insert it at head, else use insert after
    if (WhereThisGoes(head,target,true) == NULL){
        return _InsertHead(head,target);
    }
        return _InsertAfter(WhereThisGoes(head,target,true), target);
}
...
template <typename ITEM_TYPE>
node<ITEM_TYPE>* WhereThisGoes(node<ITEM_TYPE>* head, ITEM_TYPE item, bool){
    ...
    while (item > w->_item && w != NULL){
        w = w->next;
    }
    return _PreviousNode(head, w);                            //returns null if the first element
                                                            //  is getting returned
